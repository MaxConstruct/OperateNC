...
# %%
def create_time_dim(ds):
    grid_time = pd.to_datetime(ds.attrs['BeginDate'] + ' ' + ds.attrs['BeginTime'])
    return ds.assign_coords(time=grid_time)
...
for i, p in enumerate(paths):
    print(f'\r{i + 1}/{s}', end='', flush=True)
    ds = xr.load_dataset(paths[0])
    mfd_arr.append(create_time_dim(ds))
    ds.close()
print('Concat...')
mf_dataset = xr.concat(mfd_arr, dim='time')
print('To netcdf...')
mf_dataset.to_netcdf(r_path.parent / 'TRMM_3B42_Daily_1998-2019.nc')
print('Done.')
